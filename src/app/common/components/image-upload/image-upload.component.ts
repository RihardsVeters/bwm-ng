    ...
    if((file = event.target.files[0]) && (file.type === 'image/png' || file.type === 'image/jpeg')){
      ...
      img.onload = function(){
        if(this.width > FileSnippet.IMAGE_SIZE.width && this.height > FileSnippet.IMAGE_SIZE.height){
          ...
          self.toastr.error(`Min width is ${FileSnippet.IMAGE_SIZE.width} px and min height is ${FileSnippet.IMAGE_SIZE.height} px`, 'Error!');
        }
      }
      img.src = URL.createObjectURL(file);
    }else{
      this.toastr.error(`unsuported File Type Only jpeg and png is allowed!`, 'Error!');
    }
  }
