...
exports.auth = function(req,res){
    ...
    User.findOne({email}, function(err,user){
        ...
        if(user.hasSamePassword(password)){
             //return JWT token because user is verified
...
exports.register = function(req,res){
    ...
    User.findOne({email}, function(err,exsistingUser){
        if(err){
            return res.status(422).send({errors: normalizeErrors(err.errors)});
        }
        if(exsistingUser){
            return res.status(422).send({errors:[{title: 'Invalid email!', detail:'This email is already registered!'}]});
        }
